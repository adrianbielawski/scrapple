...
const PlayerStats = (props) => {
  ...
  useEffect(() => {
    if (props.isCurrent) {
      playerStats.current.scrollIntoView({
        behavior: "smooth",
        block: "nearest",
        inline: "start",
      });
  ...
  return (
    <Card className={playerStatsClass} ref={playerStats}>
      <div className={styles.wrapper}>
        <div className={styles.profileImage}>
          {props.player.user.image ? (
            <img
              src={`${BASE_URL}${props.player.user.imageThumbnail}`}
        ...
        <div className={styles.player}>
          <p className={styles.playerName}>{props.player.user.username}</p>
          <div className={styles.points}>
            <p>
              {t("Current score")} {props.player.score}
            </p>
      ...
      {displayPoints && (
        <AllPoints player={props.player} closing={closingPoints} />
      )}
    </Card>
  );
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    getAllPoints: (gameId, playerId) =>
      dispatch(getAllPoints(gameId, playerId)),
