...
const TwoLetterWords = (props) => {
  ...
  let buttonName = props.showWords
    ? "Hide two-letter words"
    : "Show two-letter words";
  ...
  useEffect(() => {
    ...
    if (props.isTouchDevice && props.deviceOrientation === "portrait-primary") {
      ...
      props.isTouchDevice &&
      props.deviceOrientation === "landscape-primary"
    ) {
      zoomable = false;
    }
...
const mapDispatchToProps = (dispatch) => {
  return {
    toggleShowWords: (showWords) => {
      dispatch(toggleShowWords(showWords));
    },
  };
};
