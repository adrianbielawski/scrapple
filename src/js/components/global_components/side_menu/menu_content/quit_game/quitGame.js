...
const QuitGame = (props) => {
  ...
  const handleQuitGame = () => {
    if (props.players.length === 2) {
      props.setAlert(
        "alert",
        "You can't quit this game, 2 players are required",
      );
      return;
    }
    props.setAlert(
  ...
  return (
    <div>
      <p className={styles.title} onClick={handleQuitGame}>
        {t("Quit this game")}
      </p>
    </div>
  );
...
const mapDispatchToProps = (dispatch) => {
  return {
    setAlert: (alertType, messageKey, messageValue, action, alertProps) =>
      dispatch(
        setAlert(alertType, messageKey, messageValue, action, alertProps),
      ),
  };
